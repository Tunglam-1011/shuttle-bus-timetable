...
// 顶部公告横幅：只针对"今天"的调整，未来/过期日期不显示。
// 默认只显示一行主标题，有副标题时点击展开，减少占用空间。
function updateNoticeBanner() {
  const banner = document.querySelector("#notice-banner");
  if (!banner) return;
  const now = new Date();
  const override = remoteScheduleOverrideMap[dateKey(now)];
  if (!override) {
    banner.hidden = true;
    return;
  }
  ...
  if (titleElement) {
    ...
    titleElement.textContent = `今日（${monthDay}）调整为${override.type === "weekday" ? "工作日" : "周末"}服务时刻表`;
  }

  const subtitle = override.notice && override.notice.subtitle;
  const toggleButton = document.querySelector("#notice-banner-toggle");
  const subtitleElement = document.querySelector("#notice-banner-subtitle");
  const chevron = document.querySelector("#notice-banner-chevron");
  if (subtitleElement) {
    subtitleElement.textContent = subtitle || "";
    subtitleElement.hidden = true; // 默认折叠
  }
  if (toggleButton) {
    toggleButton.disabled = !subtitle;
    toggleButton.setAttribute("aria-expanded", "false");
  }
  if (chevron) {
    chevron.textContent = subtitle ? "▾" : "";
  }
}

// 公告点击展开/收起副标题
const noticeBannerToggle = document.querySelector("#notice-banner-toggle");
if (noticeBannerToggle) {
  noticeBannerToggle.addEventListener("click", () => {
    const banner = document.querySelector("#notice-banner");
    const subtitleElement = document.querySelector("#notice-banner-subtitle");
    if (!banner || !subtitleElement || !subtitleElement.textContent) return;
    const expanded = banner.classList.toggle("expanded");
    subtitleElement.hidden = !expanded;
    noticeBannerToggle.setAttribute("aria-expanded", String(expanded));
  });
}
